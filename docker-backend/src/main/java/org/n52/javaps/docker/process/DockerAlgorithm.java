...
public class DockerAlgorithm extends AbstractAlgorithm {
    ...
    public void execute(ProcessExecutionContext context) throws ExecutionException {
        this.jobConfig = new DockerJobConfigImpl(this.dockerConfig, getDescription(),
                                                 context,
                                                 new Environment(executionUnit.getEnvironment()));
        ...
        try {
            ...
            CreateContainerCmd createHelperContainerCmd = createContainerCmd(HELPER_CONTAINER_IMAGE)
                                                                  ...
                                                                  .withCmd(createDirectories(jobConfig.getInputPath(),
                                                                                             jobConfig
                                                                                                     .getOutputPath()));
            ...
            try {
                CreateContainerCmd createContainerCmd = createContainerCmd(executionUnit.getImage())
                                                                .withEnv(jobConfig.getEnvironment().encode())
                                                                .withVolumes(dataVolume)
                                                                .withHostConfig(HostConfig.newHostConfig()
                                                                                          .withBinds(dataVolumeBind));
                ...
                waitForCompletion(jobConfig.getProcessContainerId(), dockerConfig.getTimeout().orElse(DEFAULT_TIMEOUT));
    ...
    private InspectContainerResponse getExitCode(String containerId) throws InterruptedException {
        ...
        long waitTime = dockerConfig.getTimeout().orElse(DEFAULT_TIMEOUT).toMillis();
    ...
    private void removeVolume(String volumeId) {
        if (volumeId != null) {
            try {
                dockerConfig.getClient().removeVolumeCmd(volumeId).exec();
    ...
    private void removeContainer(String containerId) {
        if (containerId != null) {
            try {
                dockerConfig.getClient().removeContainerCmd(containerId).withForce(true).exec();
            } catch (DockerException ex) {
    ...
    private InspectContainerResponse inspectContainer(String containerId) {
        return dockerConfig.getClient().inspectContainerCmd(containerId).exec();
    }

    private String createVolume() {
        return dockerConfig.getClient().createVolumeCmd().exec().getName();
    }

    private void startContainer(String containerId) {
        getJobLog().debug("Starting container {}", containerId);
        dockerConfig.getClient().startContainerCmd(containerId).exec();
    ...
    private CreateContainerCmd createContainerCmd(String image) {
        CreateContainerCmd command = dockerConfig.getClient().createContainerCmd(image);
        dockerConfig.getUser().map(user -> dockerConfig.getGroup()
    ...
    private Map<String, String> createLabels() {
        ...
        labels.put(Labels.JOB_ID, this.jobConfig.getContext().getJobId().getValue());
        ...
        labels.put(Labels.PROCESS_ABSTRACT, getDescription().getAbstract().map(OwsLanguageString::getValue)
                                                            .orElse(""));
        labels.put(Labels.PROCESS_VERSION, getDescription().getVersion());
        labels.put(Labels.VERSION, "1.4.0");
        return labels;
    }
    ...
    private LogContainerResultCallback createCallback(String containerId) {
        return dockerConfig.getClient().logContainerCmd(containerId)
    ...
    private List<OutputDefinition> getOutputDefinitions() {
        return getDescription().getOutputs().stream()
                               .map(this.jobConfig.getContext()::getOutputDefinition)
